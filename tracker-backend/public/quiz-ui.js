(() => {
  ...
  function gate(d){const a=activity(d);return a.lessonRead===true&&Number(a.quiz?.score||0)>=70&&progress(d)>70;}
  function enforceGate(d){
    if(!d||d>=30)return;
    const ok=gate(d);
    const next=document.getElementById("nextDayBtn");
    if(next){next.disabled=!ok;next.classList.toggle("gate-locked",!ok);next.title=ok?"Next lesson unlocked":"Pass the MCQ with at least 70% and reach more than 70% practice progress";next.innerHTML=ok?`Next ${typeof ICO!=="undefined"?ICO.arrowR:"→"}`:`Next ${typeof ICO!=="undefined"?ICO.lock:"🔒"}`;}
    const toNext=document.getElementById("toNextLesson");
    if(toNext){toNext.disabled=!ok;toNext.classList.toggle("gate-locked",!ok);toNext.title=ok?"Next lesson unlocked":"Complete the 70% MCQ and >70% progress requirements";if(!ok)toNext.textContent="Next lesson locked";}
  }
  function addBackButton(stack){
    if(stack.querySelector("[data-back-lesson]"))return;
    const wrap=document.createElement("div");wrap.className="lesson-return";wrap.innerHTML=`<button type="button" class="btn-ghost" data-back-lesson>← Back to Lesson</button>`;stack.appendChild(wrap);
    wrap.firstElementChild.onclick=()=>{store.dayTab="lesson";render();};
  }
  async function mountQuiz(d,stack){
    if(mounted)return; mounted=true;
    const card=document.createElement("div");card.className="card mcq-card";card.innerHTML=`<div class="card-head"><h2>Real-world knowledge check</h2><span class="aside">70% to pass</span></div><div class="card-body">Loading questions…</div>`;stack.appendChild(card);
    try{
      const data=await api(`/api/quiz?day=${d}`),qs=data.questions||[],body=card.querySelector(".card-body");
  ...
  function boot(){styles();const main=document.getElementById("main");if(!main)return;const observer=new MutationObserver(()=>{if(typeof store==="undefined"||store.view!=="today"){mounted=false;return;}const d=day();enforceGate(d);if(store.dayTab!=="practice")return;const stack=main.querySelector(".seg-btn[data-tab='practice']")?.closest(".seg")?.nextElementSibling?.querySelector(".col-stack");if(!stack)return;addBackButton(stack);if(!stack.querySelector(".mcq-card"))mountQuiz(d,stack);});observer.observe(main,{childList:true,subtree:true});}
